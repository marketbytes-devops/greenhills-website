...
export default function BookingFormModal() {
  ...
  useEffect(() => {
    const fetchRoomTypes = async () => {
      try {
        const response = await fetch('http://localhost:8000/api/stay/stay-page-create/');
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const formattedData = {
        ...
        checkout: data.checkout ? data.checkout.toISOString().split('T')[0] : null,
      ...
      setFormError(error.response?.data?.error || 'Failed to submit booking. Please try again.');
    } finally {
    ...
    <div className="">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        <div className="grid grid-cols-1 gap-4">
          ...
          <div>
            ...
            <input
              ...
              {...register('mobile', {
                ...
                pattern: { value: /^\d{10,15}$/, message: 'Enter a valid mobile number' },
              })}
              className={`w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-secondaryLightBlack placeholder:text-gray-600 ${
          ...
          <div>
            <label htmlFor="roomType" className="block text-sm font-medium text-gray-800 mb-1">
              Room Type
            </label>
            <select
              id="roomType"
          ...
          <div>
            <label htmlFor="adults" className="block text-sm font-medium text-gray-800 mb-1">
              Adults
            </label>
            <select
              id="adults"
